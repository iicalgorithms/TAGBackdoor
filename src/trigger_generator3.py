...
class Trojan(nn.Module):
    def __init__(self, node_embedding_dim, text_embedding_dim, text_autoencoder, hidden_dim=256, layer_num=2, dropout=0.1):
        ...
        layers.append(nn.Linear(node_embedding_dim, hidden_dim))
        ...
        self.projection = nn.Linear(node_embedding_dim, text_embedding_dim)
        # Weight for combining poisoned and neighbor embeddings
        self.similarity_weight = nn.Parameter(torch.tensor(0.0001))

        
    def forward(self, node_embeddings, original_texts, original_text_embeddings, neighbor_text_embeddings, text_poison_mode="overwriting"):
        """
        """
        h = self.layers(node_embeddings)  # Input data passes through defined hidden layer sequence
        ...
        self_embeddings = self.projection(node_embeddings)  # Project poisoned embeddings to the same space as original text embeddings

        # Average neighbor embeddings
        avg_neighbor_embeddings = torch.stack([torch.mean(embs, dim=0) if embs.numel() > 0 else torch.zeros_like(node_embeddings[0]) for embs in neighbor_text_embeddings])
        ...
        # fused_embeddings = self.similarity_weight * poisoned_embeddings + (1 - self.similarity_weight) * self_embeddings
        fused_embeddings = self.similarity_weight * poisoned_embeddings + nn.Parameter(torch.tensor(20)) * self_embeddings
        ...
        if text_poison_mode == "overwriting":
            ...
            return fused_embeddings, None

        else: # appending
            # appending_texts = self.text_autoencoder.get_texts(fused_embeddings.detach())
            # combined_texts = [f"{append} {orig}" for orig, append in zip(original_texts, appending_texts)]
            # poisoned_text_embeddings = self.text_autoencoder.get_embeds(combined_texts)
            # return poisoned_text_embeddings, appending_texts
            poisoned_text_embeddings = original_text_embeddings + fused_embeddings
            return poisoned_text_embeddings, None
...
class TextBackdoor:
    def __init__(self, config: ExperimentConfig, text_autoencoder):
        ...
        self.text_poison_mode = config.text_poison_mode
            ...
            # 预先加载 perplexity 模型和分词器，避免重复加载
        self.perplexity_model = None
        self.perplexity_tokenizer = None
        self.perplexity_cache = {}  # 添加缓存以避免重复计算
        
    def _load_perplexity_model(self):
        """懒加载 perplexity 模型和分词器"""
        if self.perplexity_model is None or self.perplexity_tokenizer is None:
            self.logger.info("Loading perplexity model and tokenizer...")
            self.perplexity_tokenizer = GPT2Tokenizer.from_pretrained('gpt2')
            # 使用半精度浮点数减少内存使用
            self.perplexity_model = GPT2LMHeadModel.from_pretrained('gpt2', torch_dtype=torch.float16).to(self.device)
            self.perplexity_model.eval()  # 确保模型处于评估模式
    ...
    def _train_models(self, data, poison_candidates, poison_mask):
        ...
        # print(
        #     f"data.x shape: {data.x.shape}, data.y shape: {data.y.shape}, data.x memory: {data.x.element_size() * data.x.nelement() / 1024 / 1024:.2f} MB, "
        #     f"data.raw_texts memory: {sum(len(t.encode('utf-8')) for t in data.raw_texts) / 1024 / 1024:.2f} MB"
        #     f"\ndata: {sys.getsizeof(data) / 1024 / 1024:.2f} MB")
        # print(f"Current GPU memory allocated: {torch.cuda.memory_allocated(self.device) / 1024 / 1024:.2f} MB")
        # print(f"Current GPU memory reserved: {torch.cuda.memory_reserved(self.device) / 1024 / 1024:.2f} MB")
        # print(f"Current GPU memory free: {(torch.cuda.get_device_properties(self.device).total_memory - torch.cuda.memory_reserved(self.device)) / 1024 / 1024:.2f} MB")
        # print(f"Current device: {self.device}")
        # data = data.to(self.device, exclude_keys=['raw_texts'])
        ...
        self.trojan = Trojan(
            node_embedding_dim=node_embedding_dim,  # input
        ...
        for epoch_trojan in range(self.config.trojan_epochs):
            ...
            original_text_embeddings=data.x[poison_mask].detach()
            ...
            poisoned_text_embeddings, _ = self.trojan(node_embeddings, original_texts, original_text_embeddings,outer_original_neighbor_text_embeddings, text_poison_mode=self.text_poison_mode)
            ...
            outer_original_text_embeddings = data.x[outer_poison_mask].detach()
            ...
            outer_poisoned_embeddings,_ = self.trojan(outer_node_embeddings, outer_original_texts,outer_original_text_embeddings, outer_original_neighbor_text_embeddings, text_poison_mode=self.text_poison_mode)
    ...
    def _create_poisoned_dataset(self, data, poison_labels, poison_mask, train_poison_mask):
        ...
        original_texts = [data.raw_texts[idx.item()] for idx in poison_indices]
        original_text_embeddings = data.x[poison_indices].detach()
        ...
        if self.text_poison_mode=="overwriting":
            poisoned_text_embeddings, overwriting_texts = self.trojan(node_embeddings, original_texts, original_text_embeddings, original_neighbor_text_embeddings,text_poison_mode=self.text_poison_mode)

            # if overwriting_texts is None:
            #     overwriting_texts = self.text_autoencoder.get_texts(poisoned_text_embeddings)
            #
            # # update raw_texts
            ...
            poisoned_text_embeddings, appending_texts = self.trojan(node_embeddings, original_texts,original_text_embeddings, original_neighbor_text_embeddings, text_poison_mode=self.text_poison_mode)
            # if appending_texts is None:
            #     appending_texts = self.text_autoencoder.get_texts(poisoned_text_embeddings)
            #
    ...
    def _get_poisoned_test_data(self, poison_train_data, target_model):
        ...
        original_text_embeddings = poison_train_data.x[test_poison_mask].detach()
        ...
        if self.text_poison_mode == "overwriting":
            poisoned_text_embeddings, overwriting_texts = self.trojan(node_embeddings, original_texts, original_text_embeddings,neighbor_text_embeddings, text_poison_mode=self.text_poison_mode)
            ...
            if overwriting_texts:
                for idx, text in enumerate(overwriting_texts):
                    new_raw_texts[test_poison_node_indices[idx]] = text
        else: # "appending":
            poisoned_text_embeddings, appending_texts = self.trojan(node_embeddings, original_texts,original_text_embeddings, neighbor_text_embeddings, text_poison_mode=self.text_poison_mode)
            if appending_texts is None:
                appending_texts = self.text_autoencoder.get_texts(poisoned_text_embeddings)
    ...
    def _calculate_perplexity(self, texts):
        ...
        for text in texts:
            ...
            try:
                # 限制文本长度，防止超出模型最大长度限制
                max_length = 1024  # GPT-2 的最大上下文长度
                ...
                # 使用 return_tensors='pt' 获取 PyTorch 张量
                inputs = self.perplexity_tokenizer(truncated_text, return_tensors='pt', truncation=True, max_length=max_length)
                ...
                # 记录错误但继续处理其他文本
                logging.warning(f"Error calculating perplexity for text: {e}")
                continue
                
        # 返回平均困惑度，如果没有有效文本则返回 0
        return total_perplexity / valid_texts if valid_texts > 0 else 0

    def _evaluate_text_reconstruction_quality(self, poisoned_test_data, original_data):
        logging.info("Evaluating text reconstruction quality for poisoned nodes...")
        # Load perplexity model and tokenizer
        ...
        original_texts = [original_data.raw_texts[i] for i in poison_indices]
        ...
        avg_length_change = np.mean(length_change)
        ...
        print(f"===original_texts: {original_texts[:5]}")
        print(f"===poisoned_texts: {poisoned_texts[:5]}")

        print(f"===original_label: {[original_data.y[i].item() for i in poison_indices[:5]]}")
        print(f"===target_label: {[poisoned_test_data.y[i].item() for i in poison_indices[:5]]}")

        original_perplexity = self._calculate_perplexity(original_texts)
        poisoned_perplexity = self._calculate_perplexity(poisoned_texts)
        perplexity_change = poisoned_perplexity - original_perplexity
        ...
        results = {
            ...
            'poisoned_perplexity': poisoned_perplexity,
            'avg_length_change': avg_length_change,
        }

        print(f"====Perplexity results: {results}")
        return results


    # def _calculate_perplexity(self, texts, batch_size=8):
    #     """使用批处理高效计算一组文本的困惑度"""
    #     # 确保模型已加载
    ...
    #     uncached_texts = []
    #     uncached_indices = []
    #     perplexities = [0] * len(valid_texts)
    #
    #     for i, text in enumerate(valid_texts):
    #         # 使用文本的哈希值作为缓存键
    #         text_hash = hash(text)
    #         if text_hash in self.perplexity_cache:
    #             perplexities[i] = self.perplexity_cache[text_hash]
    #         else:
    ...
    #                 # 使用分词器的最大长度限制，而不是简单地截断单词
    #                 processed_texts.append(text)
    #
    #             # 批量编码文本
    #             encodings = self.perplexity_tokenizer(processed_texts,
    #                                                 return_tensors='pt',
    #                                                 padding=True,
    #                                                 truncation=True,
    #                                                 max_length=1024)
    ...
    #             # 获取每个样本的损失
    #             losses = outputs.loss
    #             if losses.dim() > 0:  # 如果是批量损失
    #                 for j, loss in enumerate(losses):
    #                     # 计算困惑度并存入结果和缓存
    #                     text_perplexity = torch.exp(loss).item()
    #                     perplexities[batch_indices[j]] = text_perplexity
    #                     self.perplexity_cache[hash(batch_texts[j])] = text_perplexity
    #             else:  # 如果是单个损失
    #                 text_perplexity = torch.exp(losses).item()
    #                 perplexities[batch_indices[0]] = text_perplexity
    #                 self.perplexity_cache[hash(batch_texts[0])] = text_perplexity
    #
    #         except Exception as e:
    #             self.logger.warning(f"Error calculating perplexity for batch: {e}")
    #             # 对于出错的批次，使用默认值
    #             for idx in batch_indices:
    #                 perplexities[idx] = 100  # 使用较高的默认困惑度值
    #
    #     # 计算平均困惑度
    #     return sum(perplexities) / len(perplexities)
    #
    # def _evaluate_text_reconstruction_quality(self, poisoned_test_data, original_data):
    #     self.logger.info("Evaluating text reconstruction quality for poisoned nodes...")
    #
    #     # 不再在这里加载模型，而是使用懒加载方法
    #
    #     # 获取毒化节点的索引
    ...
    #
    #     # 打印样本文本和标签
    #     self.logger.info(f"Sample original texts: {original_texts[:2]}")
    #     self.logger.info(f"Sample poisoned texts: {poisoned_texts[:2]}")
    #     self.logger.info(f"Sample original labels: {[original_data.y[i].item() for i in poison_indices[:2]]}")
    #     self.logger.info(f"Sample target labels: {[poisoned_test_data.y[i].item() for i in poison_indices[:2]]}")
    #
    #     # 计算困惑度（使用批处理提高效率）
    #     original_perplexity = self._calculate_perplexity(original_texts, batch_size=16)
    #     poisoned_perplexity = self._calculate_perplexity(poisoned_texts, batch_size=16)
    #     perplexity_change = poisoned_perplexity - original_perplexity
    #
    #     # 整理结果
    #     results = {
    ...
    #     }
    #
    #     self.logger.info(f"Perplexity results: {results}")
    #     return results
    ...
    def run_trigger_generation_pipeline(self, data, poison_candidates, poison_mask, sorted_dims):
        ...
        for model_name in self.config.target_models:
            ...
            text_results = self._evaluate_text_reconstruction_quality(poisoned_test_data, data)
